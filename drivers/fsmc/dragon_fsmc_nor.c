...
/* Private macro -------------------------------------------------------------*/
#define NOR_WRITE(Address, Data)  (*(__IO uint16_t *)(Bank1_NOR2_ADDR + Address) = (Data))
...
{
  NOR_WRITE((0x5555), 0x00AA);
  NOR_WRITE((0x2AAA), 0x0055);
  NOR_WRITE((0x5555), 0x0090);

  NOR_ID->Manufacturer_Code = *(__IO uint16_t *) (0x0000);
  NOR_ID->Device_Code1 = *(__IO uint16_t *) (0x0001);
  NOR_ID->Device_Code2 = *(__IO uint16_t *) (0x000E);
  NOR_ID->Device_Code3 = *(__IO uint16_t *) (0x000F);
}
...
NOR_Status FSMC_NOR_EraseBlock(uint32_t BlockAddr)
{
  NOR_WRITE((0x5555), 0x00AA);
  NOR_WRITE((0x2AAA), 0x0055);
  NOR_WRITE((0x5555), 0x0080);
  NOR_WRITE((0x5555), 0x00AA);
  NOR_WRITE((0x2AAA), 0x0055);
  NOR_WRITE((0 + BlockAddr), 0x30);
...
NOR_Status FSMC_NOR_WriteHalfWord(uint32_t WriteAddr, uint16_t Data)
{
  NOR_WRITE((0x5555), 0x00AA);
  NOR_WRITE((0x2AAA), 0x0055);
  NOR_WRITE((0x5555), 0x00A0);
  NOR_WRITE((0 + WriteAddr), Data);
...
NOR_Status FSMC_NOR_ProgramBuffer(uint16_t* pBuffer, uint32_t WriteAddr, uint32_t NumHalfwordToWrite)
{
  ...
  NOR_WRITE((WriteAddr), 0x0025);
  NOR_WRITE((WriteAddr), (NumHalfwordToWrite - 1));
  ...
  while(currentaddress <= endaddress)
  {
    ...
    NOR_WRITE((currentaddress), *pBuffer++);
    currentaddress += 1;
  }

  NOR_WRITE((lastloadedaddress), 0x29);
...
* Return         : NOR_SUCCESS
*******************************************************************************/
NOR_Status FSMC_NOR_ReturnToReadMode()
{
  NOR_WRITE(0, 0x00F0);
